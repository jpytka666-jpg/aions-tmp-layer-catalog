...
# Milestone C — kontynuacja na znanym IP (SCP/tar, bez CIFS)
param(
    [string]$Ip = '192.168.1.186',
    [string]$RepoRoot = 'E:\server wiedzy',
    [string]$LogRoot = 'D:\AIONS_DEV\logs\milestone-c-hyperv'
)
...
function Write-Cont([string]$Msg) {
    $line = "[$(Get-Date -Format 'yyyy-MM-ddTHH:mm:ss')][CONT] $Msg"
    Write-Host $line
...
Write-Cont "=== Milestone C continue IP=$Ip ==="

if (-not (Test-Path $RepoTgz) -or ((Get-Item $RepoTgz).LastWriteTime -lt (Get-Date).AddHours(-6))) {
    Write-Cont 'Pakowanie repo (WSL tar)...'
    $wslTgz = ($RepoTgz -replace '\\', '/') -replace '^D:', '/mnt/d'
...
@{ vm_ip = $Ip; pre_reboot_exit = $ec1; post_reboot_exit = $ec2; strict_health = $health } |
