...
log "Full Milestone C start — REPO_SRC=${REPO_SRC}"

export DEBIAN_FRONTEND=noninteractive
apt-get update -qq
apt-get install -y -qq \
  sudo python3.11 python3.11-venv python3-pip git rsync curl jq adduser \
  ca-certificates dbus-user-session systemd-sysv

if ! id aions >/dev/null 2>&1; then
  adduser --disabled-password --gecos "" aions
fi

# Writable repo copy (install mutates target paths)
STAGING="/opt/aions/staging/repo"
mkdir -p "$(dirname "${STAGING}")"
if [[ ! -d "${STAGING}/runtime/host" ]]; then
  log "Copying repo to ${STAGING}"
  rsync -a --delete --exclude '.git' "${REPO_SRC}/" "${STAGING}/"
fi

HOST_DIR="${STAGING}/runtime/host"
cd "${HOST_DIR}"

# Full install WITHOUT --skip-runtime-up (requires systemd user session)
run_step install bash install_aions_host.sh --source-repo "${STAGING}"
