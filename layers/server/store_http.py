...
DEGRADED_LOG_PATH = os.path.join(os.path.dirname(__file__), "..", "runtime", "state", "aions_store_degraded.log")


def _log_degraded(reason: str) -> None:
    """Make a silent fallback to embedded mode observable: log it and append
    a line to a dedicated runtime log file so it is visible next to the other
    runtime logs, even though this process's stdout is not captured anywhere."""
    logger.error("VectorStore degraded to embedded PersistentClient: %s", reason)
    try:
        os.makedirs(os.path.dirname(DEGRADED_LOG_PATH), exist_ok=True)
        ts = datetime.now(timezone.utc).isoformat()
        with open(DEGRADED_LOG_PATH, "a", encoding="utf-8") as f:
            f.write(f"{ts} DEGRADED_TO_EMBEDDED reason={reason}\n")
    except Exception:
        logger.exception("Failed to write degraded-mode log entry")
...
class VectorStore:
    ...
    def __init__(self, persist_path: str | None = None,
        ...
        self.degraded: bool = False  # set True if we had to fall back to embedded mode
        ...
        if self.client is None:
            ...
            reason = f"ChromaDB HTTP not available at {self.host}:{self.port}, using embedded mode"
            print(f"[STORE] {reason}")
            self.degraded = True
            _log_degraded(reason)
    ...
    def _connect_http(self) -> chromadb.HttpClient | None:
        ...
        try:
            ...
            logger.error("ChromaDB HTTP connection failed: %s", e)
            return None
